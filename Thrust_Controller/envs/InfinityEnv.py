...
from gym_pybullet_drones.envs.BaseRLAviary import BaseRLAviary, ActionType, ObservationType
from gym_pybullet_drones.control.DSLPIDControl import DSLPIDControl


class TrajectoryEnv(BaseRLAviary):
    ...
    def __init__(self,
        ...
        path = '/home/youssef/Youssef_Mahran_2024_Quadcopter_RL/SAC_Position_Controller/gym_pybullet_drones/envs/trajectories/traj.csv'
    ...
    def _computeReward(self):
        ...
        self._target(opttarget=self.points[self.waypoint] , random=True)
        if self.first_call :
            p.loadURDF("sphere2red_nocol.urdf",
                   self.drone_target,
                   p.getQuaternionFromEuler([0, 0, 0]),
    ...
    def _disToTarget(self):
        state = self._getDroneStateVector(0)
        distance = np.linalg.norm(self.drone_target-state[0:3])
        return distance
    ...
    def reset(self,
        ...
        self.INIT_XYZS= np.array([[0.5,-1,1.75]])
        self._housekeeping()
